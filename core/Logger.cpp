...
std::streamoff Logger::s_maxFileSize = 10 * 1024 * 1024;
LogLevel Logger::s_printLevel = LogLevel::INFO;
...
{
    if (ws) {
        int len = static_cast<int>(std::wcslen(ws));
        int needed = ::WideCharToMultiByte(CP_UTF8, 0, ws, len, nullptr, 0, nullptr, nullptr);
        if (needed > 0) {
            std::string out(static_cast<size_t>(needed), '\0');
            int written = ::WideCharToMultiByte(CP_UTF8, 0, ws, len, out.data(), needed, nullptr, nullptr);
            if (written > 0) {
...
{
    int len = static_cast<int>(wsv.size());
    int needed = ::WideCharToMultiByte(CP_UTF8, 0, wsv.data(), len, nullptr, 0, nullptr, nullptr);
    if (needed > 0) {
        std::string out(static_cast<size_t>(needed), '\0');
        int written = ::WideCharToMultiByte(CP_UTF8, 0, wsv.data(), len, out.data(), needed, nullptr, nullptr);
        if (written > 0) {
            m_data.write(out.data(), static_cast<std::streamsize>(out.size()));
        }
    }
...
{
    int len = static_cast<int>(ws.size());
    int needed = ::WideCharToMultiByte(CP_UTF8, 0, ws.data(), len, nullptr, 0, nullptr, nullptr);
    if (needed > 0) {
        std::string out(static_cast<size_t>(needed), '\0');
        int written = ::WideCharToMultiByte(CP_UTF8, 0, ws.data(), len, out.data(), needed, nullptr, nullptr);
        if (written > 0) {
            m_data.write(out.data(), static_cast<std::streamsize>(out.size()));
        }
    }
